      ...
      <el-card>
        ...
        <div class="search-section">
          <el-row :gutter="20">
            ...
            <el-col :span="4">
              <el-select v-model="searchForm.type" placeholder="费用类型" clearable>
        ...
        <!-- 费用列表 -->
        <el-table :data="expenses" style="width: 100%; margin-top: 20px;" fit="true">
          ...
          <el-table-column prop="type" label="费用类型" min-width="120">
            <template #default="scope">
              <el-tag v-if="scope.row.type === 'equipment'" type="primary">设备采购</el-tag>
              <el-tag v-else-if="scope.row.type === 'software'" type="success">软件许可</el-tag>
              <el-tag v-else-if="scope.row.type === 'personnel'" type="warning">人员费用</el-tag>
              <el-tag v-else-if="scope.row.type === 'travel'" type="info">差旅费用</el-tag>
              <el-tag v-else type="default">其他费用</el-tag>
            </template>
          </el-table-column>
          ...
          <el-card class="info-card" shadow="never">
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="12">
                <div class="detail-item">
                  <label>费用类型：</label>
                  <el-tag v-if="currentExpense.type === 'equipment'" type="primary">设备采购</el-tag>
                  <el-tag v-else-if="currentExpense.type === 'software'" type="success">软件许可</el-tag>
                  <el-tag v-else-if="currentExpense.type === 'personnel'" type="warning">人员费用</el-tag>
                  <el-tag v-else-if="currentExpense.type === 'travel'" type="info">差旅费用</el-tag>
                  <el-tag v-else type="default">其他费用</el-tag>
                </div>
              </el-col>
            </el-row>
  ...
  const mockExpenses = [
    {
      ...
      approver: '',
      description: '购买高性能GPU服务器用于AI模型训练，提升算法性能。'
    },
    {
      id: 'exp002',
      projectName: '移动端开发项目',
      ...
      approver: '王经理',
      description: '购买移动端开发工具和第三方SDK授权。'
    },
    {
      id: 'exp003',
      projectName: '产品设计项目',
    ...
    },
    {
      ...
      approver: '孙经理',
      description: '参加行业技术交流会议，包括差旅费和会议费。'
    }
  ]
  ...
  const handleSearch = () => {
    ...
    if (searchForm.value.keyword) {
      ...
      filteredExpenses = filteredExpenses.filter(expense => 
        expense.projectName.toLowerCase().includes(keyword) ||
        expense.applicant.toLowerCase().includes(keyword) ||
    ...
    if (searchForm.value.status) {
      filteredExpenses = filteredExpenses.filter(expense => 
        expense.status === searchForm.value.status
      )
    }
    ...
    if (searchForm.value.type) {
      filteredExpenses = filteredExpenses.filter(expense => 
        expense.type === searchForm.value.type
      )
    }
    ...
    if (searchForm.value.dateRange && searchForm.value.dateRange.length === 2) {
      ...
      filteredExpenses = filteredExpenses.filter(expense => {
        const applyDate = expense.applyTime.split(' ')[0] // 只取日期部分
    ...
    if (filteredExpenses.length === 0) {
      ElMessage.info('没有找到匹配的费用记录')
    } else {
      ElMessage.success(`找到 ${filteredExpenses.length} 条费用记录`)
    }
  }
